...
def test_sampler_sample(mock_makedirs, mock_to_csv, mock_parallel_processor):
    model = MockModel(parameters={"param1": 1, "param2": 2})
    sampler = Sampler(
        model=model, output_folder="test_folder", cpu_count=2, batchsize=2
    )
...
def test_sampler_extract_multiindex(mock_read_csv):
    ...
    y = pd.DataFrame(
        ...
        columns=["A", "B", "C"],
        index=pd.MultiIndex.from_tuples([(0, 0), (0, 1), (1, 0)], names=["i", "j"]),
    )
...
def test_sampler_save(mock_pickle_dump, mock_open):
    ...
    mock_pickle_dump.assert_called_once_with(
        sampler, mock_open.return_value.__enter__.return_value
    )
