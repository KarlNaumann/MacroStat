...
class TorchModel(model.Model, torch.nn.Module):
    ...
    def __init__(
        ...
        model.Model.__init__(self, parameters=parameters, hyper_parameters=hyper_parameters, name=name)
    ...
    def _update_tparam(self):
        ...
        for k, v in self.parameters.items():
            self.tparam[k] = torch.tensor(
                v, dtype=torch.float64, requires_grad=True,
    ...
    def diffwhere(self, condition, x1, x2):
        ...
        if self.hyper_parameters['diffwhere']:
            sig = torch.sigmoid(torch.mul(condition, self.hyper_parameters['sigmoid_constant']))
    ...
    def tanhmask(self, x):
        ...
        return torch.div(
            torch.add(
                ...
                torch.tanh(
                    torch.mul(x, self.hyper_parameters['tanh_constant'])
                )
            ),
